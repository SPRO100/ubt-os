...
A30 higgsfield_agent (UGC / Shorts / Карусель) ── A34 caption + A35 TTS при нужде
                  ↓
media_storage: перенос из temp CDN провайдера в своё Storage (projects/{project}/{account})
                  ↓
[опционально] video_uniqualizer: 1 готовое видео → по копии на каждый другой
              аккаунт того же проекта (джиттер скорости/зума/яркости, без дублей)
                  ↓
...
### 🗂 Жизненный цикл видео: оригиналы отдельно, копии не плодятся (июль 2026)

Продолжение уникализатора — чтобы галерея не превращалась в кашу из
одинаковых роликов:

- **Общая галерея показывает только оригиналы** (`parent_video_id IS NULL`);
  копии видны исключительно в карточке проекта, свёрнутые под своим
  оригиналом («12 копий ▾»). Плюс фильтры по проекту/аккаунту/платформе/статусу.
- **Копии живут 24 часа** (`videos.expires_at`, `deploy/12_patch_video_lifecycle.sql`):
  почасовой n8n-крон (`video-cleanup.json` → `POST /video/cleanup-expired`)
  удаляет файл из Storage и помечает строку `status='expired'` —
  запись остаётся как аудит-след («у аккаунта была копия оригинала X»).
- **Без дублей**: повторный клик «На все аккаунты» пропускает аккаунты, у
  которых уже есть живая копия этого оригинала.
- **Каскадное удаление видео**: не устроил оригинал — `POST /video/delete`
  удаляет его вместе со всей веткой копий и их публикациями (`dry_run`
  сначала показывает, сколько всего удалится).

### 🎞 Своя папка для видео + уникализатор на аккаунты проекта (июль 2026)

Раньше готовое видео хранилось по временной CDN-ссылке провайдера
(Higgsfield/fal/Pexels) — ссылка истекала, а ролики разных проектов
не были никак разделены. Теперь:

- **`accounts.project_id`** (`deploy/11_patch_video_uniqualizer.sql`) — один
  аккаунт принадлежит ровно одному проекту (тому же, что и в разделе
  «Проекты»). Указывается при добавлении аккаунта.
- **`ubt_os/core/media_storage.py`** — единая точка загрузки в Supabase
  Storage; `HiggsFieldWorker` сразу после генерации перекладывает
  провайдерский temp URL в своё хранилище под
  `projects/{project_id}/{account_id}/…`, так что видео не теряются и не
  сваливаются в одну кучу.
- **Уникализатор** (`ubt_os/pipelines/video_uniqualizer.py`, `POST
  /video/uniqualize`) — из одного готового ролика делает по отдельной
  переэнкоженной копии на каждый ДРУГОЙ активный аккаунт того же проекта:
  джиттер скорости (±3%), лёгкий зум-кроп, яркость/контраст, случайное
  зеркалирование, сброс метаданных — достаточно, чтобы площадки не считали
  ролики дубликатами, без потери сути контента. Публикация остаётся
  отдельным ручным шагом.
- **Дашборд**: раздел «Проекты» → карточка проекта показывает привязанные
  аккаунты и их видео с кнопкой «🧬 На все аккаунты» у готового оригинала;
  раздел «Аккаунты» получил селектор проекта при добавлении.
...
pytest tests/ -v                            # 137 тестов   ✅ зелёные
...
│   ├── core/                # circuit breaker, budget guard, pipeline lock, risk engine, kb_writer, media_storage
│   ├── pipelines/           # Higgsfield queue/worker, video_uniqualizer, DLQ, social_publisher
...
│   ├── 11_patch_video_uniqualizer.sql # accounts.project_id, videos.account_id/parent_video_id
│   └── 12_patch_video_lifecycle.sql   # videos.expires_at, status 'expired' (24ч TTL копий)
├── n8n/workflows/           # воркфлоу (JSON), включая video-cleanup.json (почасовой крон)
├── docs/                    # аудиты и отчёты ресерча
├── tests/                   # pytest (137 тестов)
